...
# ─────────────────────────────────────────────
#   KONFIGURACE — nahrazeno při flashování
# ─────────────────────────────────────────────
WIFI_SSID     = "{{WIFI_SSID}}"
...
# Statická IP (volitelné — pokud prázdné, použije DHCP)
STATIC_IP     = "{{STATIC_IP}}"
SUBNET        = "{{SUBNET}}"
GATEWAY       = "{{GATEWAY}}"
DNS           = "{{DNS}}"

# Individuální MQTT topic pro tento deck
CONFIG_TOPIC  = "dashboard/" + API_KEY + "/config"
REQUEST_TOPIC = "dashboard/" + API_KEY + "/request/config"
...
# ─────────────────────────────────────────────
#   UX LOGIKA (Zpetna vazba a UI Render)
# ─────────────────────────────────────────────
def update_display():
...
def connect_wifi():
    ...
    wlan.active(True)
    # Staticka IP konfigurace (pokud je nastavena)
    if STATIC_IP and SUBNET and GATEWAY and DNS:
...
def mqtt_callback(topic, msg):
    ...
    # Individualni config topic pro tento deck
...
def connect_mqtt():
    ...
    try:
        ...
        mqtt_client.connect()
        # Subscribe na individualni topic pro tento deck
        mqtt_client.subscribe(CONFIG_TOPIC.encode())
        request_config()
        return True
    except Exception as e:
...
def request_config():
    if mqtt_client:
        try:
            # Pozadavek na individualni konfiguraci pro tento deck
            mqtt_client.publish(REQUEST_TOPIC.encode(), b"1")
        except: pass
...
def execute_command(cmd):
    ...
    # Standardni shell prikaz pres MQTT
    if not mqtt_client:
...
def main():
    ...
    while True:
        ...
        # 1. ZPRACOVANI HARDWAROVYCH TLACITEK (HOTKEYS)
